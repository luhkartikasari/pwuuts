...
$psn = "";
if(isset($_POST['txTAMBAH'])) {
        $kode = $_POST['txKODE'];
        $nama = $_POST['txNAMA'];
        $sks = $_POST['txSKS'];  

        $sql = "INSERT INTO tbmatakuliah (kode,nama,sks,idmatkul) VALUES ('$kode','$nama','$sks','".md5($kode)."')";
        $cnn = mysqli_connect(DBHOST,DBUSER,DBPASS,DBNAME,DBPORT) or die("Gagal koneksi ke database"); 
        $hasil = mysqli_query($cnn,$sql);
        if($hasil){
            $psn = "Tambah Matakuliah Sukses,Silahkan kembali ke dashboard";
            header('location:matakuliah.php');
        }else{
            $psn = "Tambah Matakuliah Gagal,Silahkan diulang";
        }
                ...
                <form method="post" action="">
                    <div>
                        <label class="form-label"><b>KODE</b></label>
                            <input class="form-control" type="text" name="txKODE">
                    </div>
                    ...
                    <div>
                        <label class="form-label"><b>SKS</b></label>
                            <input class="form-control"  type="number" name="txSKS">
                    </div>  

                    <div>
                        <button class="btn btn-success text-light mt-3" type="submit" name="txTAMBAH" id="tambah"><b>Tambah</b></button>
                    </div>
                </form>
                </div>
            </div>
